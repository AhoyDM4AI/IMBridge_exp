...
gb = GradientBoostingClassifier()                                                   #梯度上升
gb.fit(X_train_pre, y_train)                                                        #训练

y_prob = gb.predict_proba(X_test_pre)[:, 1]                                         #预测结果为1的概率
y_pred = gb.predict(X_test_pre)                                                     #预测结果
...
model_path = './creditcard_gb_pipeline.onnx'
